...
import { useAppDispatch, useReinc } from "../redux/hooks";
import {
...
const DrawerNumberInput = (
  ...
  // @ts-ignore
  return (
    <StyledDrawerNumberInput
      value={props.value}
      onChange={(_event, value: number | null) => props.onChange(value)}
    />
  );
};
...
const NavigationItem = (
  props: PropsWithChildren<{ text: string; target: string }>,
) => {
  return (
    <Typography
      variant={"subtitle1"}
              ...
              <List>
                <ListItem>
                  <DrawerItem direction={"row"}>
                    ...
                    <DrawerNumberInput
                      value={reinc.customSkillMaxBonus}
                      onChange={(value) => {
                        dispatch(
                          setCustomSkillMaxBonus(
                            roundAbilityTrained(
                ...
                <ListItem>
                  <DrawerItem direction={"row"}>
                    ...
                    <DrawerNumberInput
                      value={reinc.customSpellMaxBonus}
                      onChange={(value) => {
                        dispatch(
                          setCustomSpellMaxBonus(
                            roundAbilityTrained(
                ...
                <ListItem>
                  <DrawerItem direction={"row"}>
                    ...
                    <DrawerNumberInput
                      value={reinc.freeLevels}
                      onChange={(value) => {
                        if (!value) value = 0;
                        if (value > MAX_LEVEL - reinc.level)
                          value = MAX_LEVEL - reinc.level;
                        dispatch(setFreeLevels(value));
                      }}
                    />
                  </DrawerItem>
                </ListItem>
                <ListItem>
                  <DrawerItem direction={"row"}>
                    ...
                    <DrawerTextInput
                      value={reinc.copyPasteSeparator}
                      onChange={(event: any) => {
                        dispatch(
                          setCopyPasteSeparator(event.target.value || ""),
                        );
                      }}
                    />
                  </DrawerItem>
