...
import { useReinc } from "../redux/hooks";
  ...
  const { guildStats, statKey, resKeys } = useMemo(() => {
    ...
    guildStats.forEach((gs) => {
      Object.entries(gs)
        ...
        .forEach((entry) => {
          const name: (typeof baseStats)[number] =
            entry[0].toLowerCase() as BaseStatName;
          if (
  ...
  return (
    <>
      ...
      <Typography variant={"body1"}>
        {" "}
        {statKey
          .filter((sk) => !sk.includes("res"))
          .map((sk) => {
            return (
              sk +
              ": " +
              guildStats
                .map((gs) => gs[sk as keyof Resistances] as number)
                .reduce((v1, v2) => {
                  return (v1 || 0) + (v2 || 0);
                }, 0) +
              " "
        ...
        {statKey
          ...
          .map((sk) => {
            return (
              ...
              ": " +
              guildStats
                .map((gs) => gs[sk as keyof Resistances])
                .reduce((v1, v2) => {
                  return (v1 || 0) + (v2 || 0);
                }, 0) +
              " "
