...
export default async function GuildParser(data: FileObject): Promise<ParserProcess> {
    ...
    const guildLevels: Record<string, GuildLevel> = {}
    ...
    async function parseLine(line: string) {
        ...
        if (levelMatch) {
            level = levelMatch
            guildLevels[level] = { stats: [], abilities: [] }
        }
        ...
        if (statText) {
            statText.split(",")?.forEach((stat) => {
                ...
                if (statName && statValue) {
                    guildLevels[level]?.stats.push({name: statName.trim(), value: parseInt(statValue)})
                }
            })
            return
        }
        ...
        if (abilityMatch) {
            ...
            if (name && type && max) {
                guildLevels[level]?.abilities.push({name, type, max: parseInt(max), cost: 0})
                return
            }
        }
